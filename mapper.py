...
class Mapper:
	...
	def process_map(self):
		...
		# Wwise 外部ID 标准为 16 hex（8字节）。若 map 的 key 比这更长，
		# 说明前面多了前缀字节（4.5 起首字节存语言等元数据）。
		# 自动取最后 16 位作为 pck 外部ID 的回退键，前缀字节数再变也能适配。
		self.short_keys = {}
		for k in self.keys:
			if len(k) > 16:
				self.short_keys.setdefault(k[-16:], k)
	...
	def get_key(self, key, addLang=False):
		if self.mode == "MAP":
			if key not in self.keys and key in self.short_keys:
				key = self.short_keys[key]
			...
			if self.map_version == b"\x33\x31":
				if len(key) > 16:
					# 新格式：key 带前缀字节（语言等），offset=整个 3 字节 value，
					# 前缀字节数自动从 key 长度推算。
					prefix_bytes = (len(key) - 16) // 2
					lang = int(key[:prefix_bytes * 2], 16)
					offset = self.keys[key]
				else:
